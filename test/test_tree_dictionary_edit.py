...
def test_tree_dictionary_edit():
    """测试树形结构中编辑子节点时父级项目显示"""
    
    print("=== 测试树形结构中编辑子节点时父级项目显示 ===")
    ...
    print("\n2. 创建父级字典项...")
    parent_data = {
        "code": "test_tree_parent",
        "name": "测试树形父级项目",
        "description": "用于测试树形结构的父级字典项",
        "sort_order": 1,
        "is_active": True
    }
    ...
    child_data = {
        "parent": parent_id,
        "code": "test_tree_child",
        "name": "测试树形子级项目",
        "description": "用于测试树形结构的子级字典项",
        "sort_order": 1,
        "is_active": True
    }
    ...
    try:
        tree_response = requests.get(
            f"{API_BASE}/dictionaries/tree/", 
            headers=headers
        )
        tree_response.raise_for_status()
        tree_data = tree_response.json()
        print(f"✓ 获取树形结构成功")
        print(f"  树形数据: {json.dumps(tree_data, indent=2, ensure_ascii=False)}")
    ...
    try:
        list_response = requests.get(
            f"{API_BASE}/dictionaries/?include_parents=true&tree=true", 
            headers=headers
        )
        ...
        if child_item:
            ...
            print(f"  完整数据: {json.dumps(child_item, indent=2, ensure_ascii=False)}")
        else:
            print("✗ 未找到子级项目")
    ...
    try:
        ...
        print(f"  完整数据: {json.dumps(detail, indent=2, ensure_ascii=False)}")
